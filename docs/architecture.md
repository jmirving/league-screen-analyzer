...
`league-replay-v1` retains canonical synthetic seven-segment masks. `ClockProfileCatalog` is shared by the CLI and WPF composition roots. It combines built-ins with validated template manifests, rejects duplicate stable IDs, resolves base-profile inheritance deterministically, and exposes display name, version, provenance, template count, and source manifest. Packaged profiles live at `fixtures/clocks` beside the executable; user-installed profiles live at `%LOCALAPPDATA%\LeagueScreenAnalyzer\profiles`; `LEAGUE_SCREEN_ANALYZER_CLOCK_PROFILES` is the explicit development override. An upward repository search is used only when packaged manifests are absent. Template-backed profiles support multiple provenance-tracked templates per glyph and use nearest-template overlap scoring with one-pixel translation tolerance. Candidate confidence includes absolute glyph quality and first/second margin; the weakest glyph can make the whole clock unavailable. Calibration evaluation is independent-sample by construction and emits separate apparent-training and leave-one-sample-out reports without changing normal `ReplayContinuous` validation.
